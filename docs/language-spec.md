...
Теперь точкой входа является процедура или функция `main` без параметров.
Всесторонний исполняемый код должен находиться внутри `main`; верхнеуровневые операторы не допускаются.
