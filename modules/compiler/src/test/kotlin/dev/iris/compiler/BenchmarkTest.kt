...
class BenchmarkTest {
    ...
    @Test
    fun `benchmark quicksort 100000 elements`() {
        val source = """
            функция: целч разделить(целч* массив, целч левый, целч правый)
            нч
                целч средний = левый + (правый - левый) / 2;

                если (массив[левый] > массив[средний])
                нч
                    целч т1 = массив[левый];
                    массив[левый] = массив[средний];
                    массив[средний] = т1;
                кц
                если (массив[левый] > массив[правый])
                нч
                    целч т2 = массив[левый];
                    массив[левый] = массив[правый];
                    массив[правый] = т2;
                кц
                если (массив[средний] > массив[правый])
                нч
                    целч т3 = массив[средний];
                    массив[средний] = массив[правый];
                    массив[правый] = т3;
                кц

                целч т4 = массив[средний];
                массив[средний] = массив[правый - 1];
                массив[правый - 1] = т4;

                целч опорный = массив[правый - 1];
                целч слева = левый;
                целч справа = правый - 1;

                пока (слева < справа)
                нч
                    слева = слева + 1;
                    пока (массив[слева] < опорный)
                    нч
                        слева = слева + 1;
                    кц
                    справа = справа - 1;
                    пока (массив[справа] > опорный)
                    нч
                        справа = справа - 1;
                    кц
                    если (слева < справа)
                    нч
                        целч т5 = массив[слева];
                        массив[слева] = массив[справа];
                        массив[справа] = т5;
                    кц
                кц

                целч т6 = массив[слева];
                массив[слева] = массив[правый - 1];
                массив[правый - 1] = т6;

                вернуть слева;
            кц

            процедура быстраяСортировка(целч* массив, целч левый, целч правый)
            нч
                если (левый + 10 < правый)
                нч
                    целч опорныйИндекс = разделить(массив, левый, правый);
                    быстраяСортировка(массив, левый, опорныйИндекс - 1);
                    быстраяСортировка(массив, опорныйИндекс + 1, правый);
                кц
                иначе
                нч
                    целч внеш = левый + 1;
                    пока (внеш <= правый)
                    нч
                        целч врем = массив[внеш];
                        целч внут = внеш;
                        пока ((внут > левый) и (массив[внут - 1] > врем))
                        нч
                            массив[внут] = массив[внут - 1];
                            внут = внут - 1;
                        кц
                        массив[внут] = врем;
                        внеш = внеш + 1;
        ...
        assertEquals("1", output[0], "Array should be sorted")
        assertEquals("1", output[1], "First element should be 1")
        assertEquals("100000", output[2], "Last element should be 100000")
        println("QuickSort 100000 elements: sorted=${output[0]}, first=${output[1]}, last=${output[2]}, time: ${time}ms")
    }

    @Test
    fun `benchmark quicksort 1000000 elements`() {
        val source = """
            функция: целч разделить(целч* массив, целч левый, целч правый)
            нч
                целч средний = левый + (правый - левый) / 2;
        ...
        assertEquals("1", output[0], "Array should be sorted")
        assertEquals("1", output[1], "First element should be 1")
        assertEquals("1000000", output[2], "Last element should be 1000000")
        println("QuickSort 1000000 elements: sorted=${output[0]}, first=${output[1]}, last=${output[2]}, time: ${time}ms")
    }
